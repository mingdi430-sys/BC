...
const code=fs.readFileSync(new URL('../src/data.js',import.meta.url),'utf8').replace(/^\uFEFF?import source from .*;\s*/,'').replace(/export /g,'');
const api=new Function('source',code+'\nreturn {getRecords,municipalityGroups,resolveRegion};')(raw);
