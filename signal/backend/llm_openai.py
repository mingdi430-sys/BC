"""OpenAI(gpt-5-mini) 기반 도우미: 화면 조작 도구 + 카드 데이터 SQL 질의 도구.

- 자연어 → SQL → 실행 결과를 근거로 답한다 (숫자는 결과에서만).
- 범위 밖 질문(예측·추천·창업 성패 판단·원인 분석·데이터 밖 지식·개인정보)은 정해진 문구로 거절한다.
- OpenAI 호환 엔드포인트면 Gemini(OpenAI 호환 API)도 LLM_BASE_URL 로 붙는다.
"""
...
불가: 미래 예측, 창업 추천이나 성공/실패 판단, 원인·이유 해석, 데이터에 없는 정보(임대료·유동인구·경쟁 등), 개인 식별, 데이터와 무관한 잡담.
        ...
        for tc in msg.tool_calls:
            ...
            if name == "query_card_data":
                try:
                    ...
                    queries.append({"sql": res["sql"], "purpose": args.get("purpose", ""), "rows": len(res["rows"])})
