...
**인구 1인당 지표는 넣지 않습니다.** 그 지역 결제 총액에는 거주자뿐 아니라 방문객·유동인구 소비도 섞여 있어, 거주 인구(행정안전부 주민등록 인구)로 나누면 "주민 소비력"처럼 보이지만 실제로는 관광지·환승지 등 외부 유입이 많은 지역이 왜곡되어 높게 나옵니다. 오해를 줄 수 있는 지표라 제외했습니다(과거에 `perCapitaAmount`/`populationData.json`로 있었으나 제거함). 12개월 이상 계절성 비교도 이 데이터로는 할 수 없어 포함하지 않았습니다.
